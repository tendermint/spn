...
func TestGenesisAccountQueryPaginated(t *testing.T) {
	keeper, _, ctx, _ := setupKeeper(t)
	wctx := sdk.WrapSDKContext(ctx)
	msgs := createNGenesisAccountForChainID(keeper, ctx, 5, "foo")
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(msgs); i += step {
			resp, err := keeper.GenesisAccountAll(wctx, request("foo", nil, uint64(i), uint64(step), false))
			require.NoError(t, err)
	...
	t.Run("ByKey", func(t *testing.T) {
		...
		var next []byte
		for i := 0; i < len(msgs); i += step {
			resp, err := keeper.GenesisAccountAll(wctx, request("foo", next, 0, uint64(step), false))
			require.NoError(t, err)
			for j := i; j < len(msgs) && j < i+step; j++ {
				assert.Equal(t, &msgs[j], resp.GenesisAccount[j-i])
			}
	...
	t.Run("Total", func(t *testing.T) {
		resp, err := keeper.GenesisAccountAll(wctx, request("foo", nil, 0, 0, true))
		require.NoError(t, err)
