...
func TestMsgRevertLaunch(t *testing.T) {
	...
	chain.LaunchTimestamp = testkeeper.ExampleTimestamp.Unix() - int64(types.DefaultRevertDelay) + 1
	...
	require.True(t, found)
	chain.LaunchTriggered = true
	chain.LaunchTimestamp = testkeeper.ExampleTimestamp.Unix() - int64(types.DefaultRevertDelay)
	k.SetChain(sdkCtx, chain)
