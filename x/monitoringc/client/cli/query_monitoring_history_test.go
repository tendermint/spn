...
func TestListMonitoringHistory(t *testing.T) {
	net, objs := networkWithMonitoringHistoryObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListMonitoringHistory(), args)
			require.NoError(t, err)
			var resp types.QueryAllMonitoringHistoryResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
