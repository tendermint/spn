...
func networkWithLaunchIDFromChannelIDObjects(t *testing.T, n int) (*network.Network, []types.LaunchIDFromChannelID) {
	t.Helper()
	...
	for i := 0; i < n; i++ {
		launchIDFromChannelID := types.LaunchIDFromChannelID{
			ChannelID: strconv.Itoa(i),
		}
		nullify.Fill(&launchIDFromChannelID)
		state.LaunchIDFromChannelIDList = append(state.LaunchIDFromChannelIDList, launchIDFromChannelID)
	}
	buf, err := cfg.Codec.MarshalJSON(&state)
	require.NoError(t, err)
	cfg.GenesisState[types.ModuleName] = buf
	return network.New(t, cfg), state.LaunchIDFromChannelIDList
}

func TestShowLaunchIDFromChannelID(t *testing.T) {
	net, objs := networkWithLaunchIDFromChannelIDObjects(t, 2)
...
func TestListLaunchIDFromChannelID(t *testing.T) {
	net, objs := networkWithLaunchIDFromChannelIDObjects(t, 5)
