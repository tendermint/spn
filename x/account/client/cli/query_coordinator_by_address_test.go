...
func TestListCoordinatorByAddress(t *testing.T) {
	net, objs := networkWithCoordinatorByAddressObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListCoordinatorByAddress(), args)
			require.NoError(t, err)
			var resp types.QueryAllCoordinatorByAddressResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
			for j := i; j < len(objs) && j < i+step; j++ {
				assert.Equal(t, objs[j], resp.CoordinatorByAddress[j-i])
			}
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.CoordinatorByAddress)
	})
}
